...
import { createBusSearchProvider } from '../../src/providers/bus-search';
...
const
  ...
  storageService: any = {
    getBusSearch: sinon.stub().returns(Promise.resolve(busSearch)),
    setBusSearch: sinon.stub(),
  },
...
describe('bus list provider', () => {
  ...
  beforeEach(() => {
    ...
    storageService.setBusSearch.resetHistory();
  });

  it('fetches last search history', done => {
    busSearchProvider.subscribe(store)
    .then(() => {
      sinon.assert.calledOnce(storageService.getBusSearch);
      done();
    })
    .catch(done);
  });
